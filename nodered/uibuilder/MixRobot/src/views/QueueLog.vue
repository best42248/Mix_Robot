<template>
  <div class="dock-container">
    <div v-for="queue in queueOptions" :key="queue.queue" class="dock">
      <div @click="toggleProductList(queue.queue)" class="dock-header">
        ...
        <!-- แสดงเวลา (Timestamp, Start Time, End Time) ในแถวเดียว -->
        <div class="time-info">
          <p><strong>Timestamp :</strong> {{ formatDate(queue.timeStamp) }}</p>
          <p><strong>Start Time :</strong> {{ formatDate(queue.startTime) }}</p>
          <p><strong>End Time :</strong> {{ formatDate(queue.endTime) }}</p>
        </div>
        <button class="triangle-button">
          <span :class="isProductListVisible(queue.queue) ? 'triangle-up' : 'triangle-down'"></span>
        </button>
      </div>

      <!-- แสดงสินค้าเมื่อมีการคลิกเพื่อเปิดรายการ -->
      <div v-if="isProductListVisible(queue.queue)" class="queue-container">
        <h3>Product List</h3>
...
module.exports = {
  ...
  methods: {
    ...
    queryData() {
      uibuilder.send({ payload: {historyQueue : true} });
    },
    handleIncomingData(msg) {
      if (
        ...
        const { historyQueue } = msg.payload;

        // Verify locations and products are arrays and have the required structure
        if (Array.isArray(historyQueue)) {
          this.queueOptions = historyQueue;
        }
      }
    },
    formatDate(dateString) {
      const date = new Date(dateString);
      return date.toLocaleString(); // Formats the date in the local time
    },
...
<style scoped>
/* Styles for the time information */
.time-info {
  display: flex;
  font-size: 0.9rem;
  color: #444;
...
.time-info p {
  margin: 0; /* เอามาร์จิ้นออกจากแต่ละ p */
  line-height: 1.4;
}
.dock-container {
  display: flex;
